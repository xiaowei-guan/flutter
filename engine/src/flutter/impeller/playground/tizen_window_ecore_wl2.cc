...
time_t GetBootTimeEpoch() {
  ...
  if (clock_gettime(CLOCK_REALTIME, &now) != 0) {
    std::cout << "Fail to get clock_gettime(CLOCK_REALTIME)." << std::endl;
    return -1;
  }
  if (clock_gettime(CLOCK_BOOTTIME, &boot_time) != 0) {
    std::cout << "Fail to get clock_gettime(CLOCK_BOOTTIME)." << std::endl;
    return -1;
  }
...
std::string GetPreferenceKey(bool clear_exist_key) {
  ...
  if (ret != APP_CONTROL_ERROR_NONE || !id) {
    std::cout << "Fail to get app id." << std::endl;
    return std::string();
  }
...
void SetPointingDevicePreference() {
  ...
  if (ret != PREFERENCE_ERROR_NONE) {
    std::cout << "Fail to set toasted preference." << std::endl;
  }
}
...
bool TizenWindowEcoreWl2::CreateWindow() {
  ...
  if (!ecore_wl2_display_) {
    std::cout << "Ecore Wl2 display not found." << std::endl;
    return false;
  }
...
void TizenWindowEcoreWl2::EnableCursor() {
  ...
  *(void**)(&CursorModule_Finalize) = dlsym(handle, "CursorModule_Finalize");
  ...
  if (!CursorModule_Initialize || !Cursor_Set_Config ||
    ...
    std::cout << "Could not load symbols from the library." << std::endl;
    dlclose(handle);
    return;
  }
  ...
  if (!Cursor_Set_Config(surface, 1, nullptr)) {
    std::cout << "Failed to set a cursor config value." << std::endl;
  }
...
void TizenWindowEcoreWl2::SetPointingDeviceSupport() {
  ...
  *(void**)(&Mouse_Pointer_Support) = dlsym(handle, "Mouse_Pointer_Support");
  ...
  if (!Mouse_Pointer_Support) {
    std::cout << "Could not load symbols from the library.";
    dlclose(handle);
    return;
  }
...
int32_t TizenWindowEcoreWl2::GetDpi() {
  ...
  if (!output) {
    std::cout << "Could not find an output associated with the window.";
    return 0;
  }
...
uint32_t TizenWindowEcoreWl2::GetResourceId() {
  ...
  if (!registry) {
    std::cout << "Could not retreive wl_registry from the display.";
    return 0;
  }
  ...
  eina_iterator_free(iter);
  if (!surface) {
    std::cout << "Failed to initialize the tizen surface.";
    return 0;
  }
  ...
  if (!resource) {
    std::cout << "Failed to get tizen resource.";
  ...
  if (!event_queue) {
    std::cout << "Failed to create wl_event_queue.";
...
void TizenWindowEcoreWl2::UpdateFlutterCursor(const std::string& kind) {
  ...
  if (vconf_get_int(kSysMouseCursorPointerSizeVConfKey, &pointer_size) < 0) {
    std::cout << "Failed to load cursor size.";
  }
  ...
  if (kind == "basic") {
    ...
    std::cout << kind << " cursor is not supported.";
  }
  ...
  std::cout << "UpdateFlutterCursor is not supported.";
}
